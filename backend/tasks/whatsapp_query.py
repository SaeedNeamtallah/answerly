...
from backend.database.models import WhatsAppIntegration, Conversation, ConversationMessage, WhatsAppCustomer
from backend.monitoring.metrics import TELEGRAM_WEBHOOK_FAILURES_TOTAL as WHATSAPP_WEBHOOK_FAILURES_TOTAL
...
async def _claim_customer_message(db: AsyncSession, customer_message_id: int) -> ConversationMessage | None:
    ...
    if status == "processing":
        ...
        timeout = timedelta(seconds=max(1, int(settings.whatsapp_reply_generation_claim_timeout_seconds)))
...
async def _save_failure_reply(
    ...
    *,
    ...
    await conversation_service.save_whatsapp_message(
        db,
...
async def _generate_whatsapp_reply(customer_message_id: int) -> dict[str, object]:
    ...
    try:
        async with session_maker() as db:
            ...
            integration = await db.get(WhatsAppIntegration, int(customer_message.whatsapp_integration_id))
            conversation = await db.get(Conversation, int(customer_message.conversation_id))
            customer = await db.get(WhatsAppCustomer, int(customer_message.whatsapp_customer_id or 0))
            ...
            try:
                ...
                WHATSAPP_WEBHOOK_FAILURES_TOTAL.labels(reason="query_infrastructure").inc()
                ...
                WHATSAPP_WEBHOOK_FAILURES_TOTAL.labels(reason="processing").inc()
                fallback = integration.fallback_message or "Support is temporarily unavailable. Please try again later."
                await _save_failure_reply(
                    db,
                    integration=integration,
                    ...
                    customer_message=customer_message,
                    fallback=fallback,
                    error="WhatsApp reply generation failed",
                )
                return {"status": "fallback", "customer_message_id": int(customer_message.id)}
    finally:
...
@celery_app.task(
    bind=True,
    name="backend.tasks.whatsapp_query.generate_whatsapp_reply",
    queue="default",
)
def generate_whatsapp_reply_task(self, customer_message_id: int):
    return asyncio.run(_generate_whatsapp_reply(int(customer_message_id)))
