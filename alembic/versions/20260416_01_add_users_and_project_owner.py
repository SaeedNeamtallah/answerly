...
def upgrade() -> None:
    ...
    if _table_exists(inspector, "projects"):
        ...
        # Backfill existing project rows before making the new column NOT NULL.
        ...
        inspector = sa.inspect(bind)
        user_id_col = next(
            (col for col in inspector.get_columns("projects") if col["name"] == "user_id"),
            None,
        )
...
def downgrade() -> None:
    ...
    if _table_exists(inspector, "users"):
        # user indexes and constraints are removed with table drop
        op.drop_table("users")
